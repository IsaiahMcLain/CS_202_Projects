...
int main(){
	...
	setPrice(newBook, collection[0]); //issue
	newBook.display();
}
...
void setBooks(Book books[]){
	try{
		...
		books[2].setYear(10); //need to catch here
		books[2].setPrice(-9.98); //and here
	...
	//but the program doesn't break because we catch it here
		...
		cerr << e.what() << endl << endl; //cerr better for error statements
	}
